...
function main() {
	...
	// Compute the matrices. Gregg's code uses gl.canvas.clientWidth and
	// gl.canvas.clientHeight (there's an entire aside on this at the end
	// of the web page), it should be the same because of the canvas resize
	// function we have.
	let matrix = m3.projection(gl.canvas.width, gl.canvas.height)
	matrix = m3.translate(matrix, translation[0], translation[1])
	matrix = m3.rotate(matrix, angleRad)
	matrix = m3.scale(matrix, scale[0], scale[1])

	// Move the 'F' so that its center is at the origin
...
let m3 = {
    ...
    rotate: function(m, tx, ty) {
	return m3.multiply(m, m3.rotation(tx, ty))
    },
    scale: function(m, tx, ty) {
	return m3.multiply(m, m3.scaling(tx, ty))
    }
}
